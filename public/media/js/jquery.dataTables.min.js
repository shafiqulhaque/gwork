...
(function (j, qa, p) {
    ...
    n.oPagination = {
        two_button: {
            ...
            fnUpdate: function (g) {
                if (g.aanFeatures.p) for (var m = g.aanFeatures.p, r = 0, s = m.length; r < s; r++) if (m[r].childNodes.length !== 0) {
...
function s(a) {
    ...
    if (a.bInitialised === false) setTimeout(function () {
        ...
        b = 0;
        for (c = a.aoColumns.length; b < c; b++) if (a.aoColumns[b].sWidth !== null) a.aoColumns[b].nTh.style.width = u(a.aoColumns[b].sWidth); if (a.oFeatures.bSort) O(a);
        else {
...
function v(a, b) {
    ...
    for (var d, f, e = 0; e < b.length; e++) {
        ...
        if (a.aoColumns[e]._bAutoType && a.aoColumns[e].sType != "string") {
            ...
            if (a.aoColumns[e].sType === null) a.aoColumns[e].sType =
                f;
...
function ta(a, b) {
    ...
    for (var f = 0, e = b.aaData.length; f < e; f++) if (c) {
        ...
        v(a, i)
    } else v(a, b.aaData[f]); a.aiDisplay = a.aiDisplayMaster.slice();
        ...
        if (i == "<") {
            ...
            if (k == "'" || k == '"') {
                ...
                if (l.indexOf(".") != -1) {
                    ...
                    h.className = k[1]
                } else if (l.charAt(0) == "#") h.setAttribute("id", l.substr(1, l.length - 1)); else h.className = l;
                t += q
            }
            c.appendChild(h);
            c = h
...
function ya(a) {
    ...
    a.oScroll.bInfinite && j(f).scroll(function () {
        if (!a.bDrawing) if (j(this).scrollTop() + j(this).height() > j(a.nTable).height() - a.oScroll.iLoadGap) if (a.fnDisplayEnd() < a.fnRecordsDisplay()) {
            ea(a, "next");
            E(a);
            C(a)
        }
...
function wa(a) {
    ...
    c.keyup(function () {
        ...
        this.value != a.oPreviousSearch.sSearch && P(a, {
            sSearch: this.value, 
...
function ha(a, b) {
    ...
    for (var c = 0, d = b.length; c < d; c++) a.asDataSearch[c] =
        ka(a, a.aoData[b[c]]._aData)
}
function ka(a, b) {
    for (var c = "", d = p.createElement("div"), f = 0, e = a.aoColumns.length; f < e; f++) if (a.aoColumns[f].bSearchable) c += ja(b[f], a.aoColumns[f].sType) + "  "; if (c.indexOf("&") !== -1) {
        d.innerHTML = c;
    ...
    j(b).click(function (f) {
        if (a.aoColumns[c].bSortable !== false) {
            var e = function () {
                var i, h;
                if (f.shiftKey) {
                    for (var k = false, l = 0; l < a.aaSorting.length; l++) if (a.aaSorting[l][0] == c) {
                        k = true;
                        i = a.aaSorting[l][0];
                        h = a.aaSorting[l][2] + 1;
...
function T(a) {
    ...
    } else j(a.aoColumns[b].nTh).addClass(a.aoColumns[b].sSortingClass); e = h.sSortColumn;
...
function ea(a, b) {
    ...
    } else a._iDisplayStart = 0; else H(a, 0, "Unknown paging action: " + b);
...
function K(a, b) {
    if (a.oFeatures.bProcessing) {
        a = a.aanFeatures.r;
        for (var c = 0, d = a.length; c < d; c++) a[c].style.visibility =
            b ? "visible" : "hidden"
    } 
}
function ga(a, b) {
...
function Z(a) {
    ...
    for (i = b.length; e < i; e++) {
        ...
        h = d = 0;
        for (k = a.aoColumns.length; h < k; h++) if (a.aoColumns[h].bVisible) f.push(c[h - d]);
            else {
    ...
    if (a.oFeatures.bStateSave) {
        ...
        if (d !== null && d !== "") {
            ...
            for (f = a.aoStateLoad.length; d < f; d++) if (!a.aoStateLoad[d].fn(a,
                c)) return; a.oLoadedState = j.extend(true, {}, c);
...
function fa(a) {
    ...
    for (i =
        ...
        var t = 0, G = [];
        f = 0;
        for (h = a[d].childNodes.length; f < h; f++) if (a[d].childNodes[f].nodeName.toUpperCase() == "TD" || a[d].childNodes[f].nodeName.toUpperCase() == "TH") G.push(a[d].childNodes[f]); f = 0;
        for (h = G.length; f < h; f++) {
            var J = G[f].getAttribute("colspan") * 1, B = G[f].getAttribute("rowspan") * 1;
            if (!J || J === 0 || J === 1) {
                k = l(b, d, t);
                b[d][k] = G[f].nodeName.toUpperCase() == "TD" ? 4 : G[f];
                if (B || B === 0 || B === 1) for (e = 1; e < B; e++) {
                    q(d + e);
                    b[d + e][k] = 2
                }
                t++
...
this.fnGetPosition = function (a) {
    ...
    else if (a.nodeName.toUpperCase() == "TD") for (var c = Q(b, a.parentNode), d = 0, f = 0; f < b.aoColumns.length; f++) if (b.aoColumns[f].bVisible) {
        ...
        if (b) {
            ...
            if (b >= S(d)) {
                ...
                if (i) {
                    ...
                    l = j(">tr", d.nTFoot);
                    f = 1;
                    for (e = l.length; f < e; f++) l[f].appendChild(d.aoColumns[a].anTfExtra[f -
                        1])
                }
                f = 0;
...
this.fnDestroy = function () {
    ...
    a.bDestroying = true;
    d = 0;
    for (f = a.aoColumns.length; d < f; d++) a.aoColumns[d].bVisible === false && this.fnSetColumnVis(d,
        true);
...
for (var pa in n.oApi) if (pa) this[pa] = r(pa); this.oApi._fnExternApiFunc = r;
...
return this.each(function () {
    ...
    if (this.nodeName.toLowerCase() != "table") H(e, 0, "Attempted to initialise DataTables on a node which is not a table: " + this.nodeName);
        ...
        if (typeof g.aoColumnDefs != "undefined") for (a = g.aoColumnDefs.length - 1; a >= 0; a--) {
            ...
            c = 0;
            for (d = l.length; c < d; c++) if (typeof l[c] == "number" && l[c] >= 0) {
        ...
        if (h) for (a = 0; a < g.aaData.length; a++) v(e, g.aaData[a]); else z(e);
